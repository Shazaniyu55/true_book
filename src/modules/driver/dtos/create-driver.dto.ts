...
export class UpdateDriverTripDto {
  ...
  // ── same shape as CreateDriverTripDto (all optional) ──
  @IsString()
  @IsOptional()
  departureDate?: string;
  ...
  @IsString()
  @Matches(/^\d{2}:\d{2}(:\d{2})?$/, {
    message: 'arrivalTime must be in HH:mm or HH:mm:ss format',
  })
  arrivalTime?: string;
  ...
  @IsString()
  @Matches(/^\d{2}:\d{2}(:\d{2})?$/, {
    message: 'bookingClosingTime must be in HH:mm or HH:mm:ss format',
  })
  bookingClosingTime?: string;
