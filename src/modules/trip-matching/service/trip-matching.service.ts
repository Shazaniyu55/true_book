...
export class TripMatchingService {
  ...
  private async upsertPool(
    ...
    const dispatchAt = new Date(departureAt.getTime() - windowHours * 3_600_000);

    // Runs inside the caller's transaction (cron sweep or on-create match).
    // Reuse an existing pool for this key that's still MATCHING (not yet on
    // the board) so late arrivals join the same group.
    let pool = await em.findOne(TripRequestPool, {
      where: { matchKey, status: TripPoolStatus.MATCHING },
    });
    ...
    return em.save(TripRequestPool, pool);
  }
...
//    * Safe to run repeatedly (idempotent) — it's driven by a cron.
//    */
//   async matchPendingRequests(): Promise<{ pooled: number; poolsTouched: number }> {
//     const today = new Date().toISOString().slice(0, 10);
...
//     for (const [key, members] of buckets) {
//       const pool = await this.upsertPool(key, members);
//       if (!pool) continue; // e.g. departure already passed
//       touched.add(pool.id);
//       pooled += members.length;
//     }
