  ...
  async checkVersion(
    ...
    try {
      const settings = await this.repo.findOne({
        where: { appType, platform },
        order: { createdAt: 'DESC' },
      });

      // No settings configured yet — this is likely a transient/setup state,
      // so cache it briefly (not CACHE_TTL.LONG) so it self-heals quickly
      // once an admin adds the real row, instead of masking it for 30 min.
      if (!settings) {
        ...
        await this.cache.set(
          this.versionKey(appType, platform),
          fallbackResult,
          CACHE_TTL.SHORT, // e.g. 30s–1min — adjust to whatever short TTL your constants define
        );
      ...
      // Real settings found — safe to cache for the full duration.
      ...
      // Don't cache error fallbacks at all — a transient DB blip shouldn't
      // get locked in for any length of time.
      return {
        minVersion: fallbackVersion,
        latestVersion: fallbackVersion,
        isForceUpdate: false,
